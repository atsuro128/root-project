...
```bash
cd /root-project && echo "" | codex exec "Step N（ステップ名）の設計レビューを実施してください" --sandbox danger-full-access
```

### 設計レビュー（再レビュー）

```bash
cd /root-project && echo "" | codex exec "Step N の設計レビュー（再レビュー）を実施してください" --sandbox danger-full-access
```

### Issue 解決レビュー（コミット済み）

```bash
cd /root-project && echo "" | codex exec "issues/pending-review/ にある Issue の解決レビューを実施してください" --sandbox danger-full-access
```
...
cd /root-project && echo "" | codex exec "Issue NNN の解決レビューを実施してください。変更差分は /tmp/issue-diff.txt を参照してください。issue ファイルは dev-journal/issues/open/NNN-*.md です" --sandbox danger-full-access
```

### PR レビュー（expense-saas の実装コード）

```bash
git -C /root-project/expense-saas fetch origin && cd /root-project && echo "" | codex exec "PR #N のレビューを実施してください" --sandbox danger-full-access
```

### PR 再レビュー（指摘対応後）

```bash
git -C /root-project/expense-saas fetch origin && cd /root-project && echo "" | codex exec "PR #N の再レビューを実施してください" --sandbox danger-full-access
```
...
- **重要: バックグラウンド実行時は上記例のように `echo "" | codex exec` の形で stdin に空文字列を直接渡すこと。** そうしないと `codex exec` が `Reading additional input from stdin...` で無限待機する。
- 完了通知を受け取ったら、結果を確認する
