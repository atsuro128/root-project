# チーム構成とオーケストレーション規約

## 指揮役の制約

- **ユーザーとの対話を常に維持する**（最優先）
  - 計画立案・ファイル操作・レビュー等、会話を長く止める作業は全てサブエージェントに委譲する
  - サブエージェントは `run_in_background: true` で起動し、完了を待たずユーザーとの対話を継続する
  - エージェント完了通知を受けたら結果を報告し、次のアクションを提案する
- 成果物ファイルを直接編集しない（サブエージェントに委譲）
- レビューを自分で行わない（reviewer エージェントに委譲）
- 設計判断を独断しない（architect の分析を元にユーザーに確認）

## .claude/ ディレクトリの書き込み制限

`.claude/` は Claude Code の保護対象。バックグラウンドエージェントは確認プロンプトに応答できないため、一部パスへの書き込みが不可。

| パス | バックグラウンド | フォアグラウンド |
|------|:---:|:---:|
| `.claude/agents/`, `.claude/skills/`, `.claude/commands/` | OK | OK |
| `.claude/agent-memory/` | OK (`memory: project` で自動許可) | OK |
| `.claude/rules/` | **NG** | OK |
| `.claude/settings.json` | **NG** | OK |

**対処**: `.claude/rules/` の更新は指揮役がフォアグラウンドで直接実行する。バックグラウンドエージェントに委譲しない。

## 品質ゲート判定基準

| reviewer 結果 | 判定 | アクション |
|--------------|------|-----------|
| blocker なし | PASS | 次のタスクに進む |
| blocker あり（設計者で修正可能） | FIX | 該当エージェントに修正指示 → 再レビュー |
| blocker あり（上流の問題） | ESCALATE | `/issue 起票` → ユーザーに判断を求める |
| warning のみ | PASS with NOTE | warning を記録して次のタスクに進む |
| 3回修正しても未解消 | ESCALATE | ユーザーにエスカレート |

## 意思決定権限

### 指揮役が単独で行えること
- サブエージェントの起動・再起動
- タスク実行順序の決定
- reviewer PASS 時の次タスク進行
- warning の記録して続行
- issue の起票
- progress.md / task-plans/ の更新

### ユーザー承認が必須なこと
- コミット・PR のマージ
- blocker の修正方針
- 上流成果物の修正
- 作業計画への合意
